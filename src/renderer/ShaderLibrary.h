...
/// ShaderLibrary v4 — PBR + Hemisphere Ambient + Poisson Shadows + Specular AA
...
// ════════════════════════════════════════════════════════════════
// PBR v4 — Cook-Torrance + Hemisphere + Poisson Shadows
// ════════════════════════════════════════════════════════════════
...
out float vViewDepth;             // For cascade selection
...
void main() {
    ...
    vViewDepth = -viewPos.z;  // Positive depth into screen
...
in vec3 vNormal;
in vec2 vTexCoord;
in vec4 vFragPosLightSpace[2];  // CSM: 2 cascades
in float vViewDepth;
out vec4 FragColor;
...
uniform float uCascadeSplit;  // View-space depth where cascade 0 ends
uniform vec3 uViewPos;
uniform mat4 uLightSpaceMatrix[2];  // Needed for volumetric raymarching
uniform int uRenderMode;  // 0=PBR, 1=Toon, 2=Neon

// ════════════════════════════════════════════════════════════
// PBR BRDF Functions — Production Quality
// ════════════════════════════════════════════════════════════

// Specular Anti-Aliasing: reduces sparkling highlights on normal-mapped surfaces
// by widening the roughness based on normal derivatives
float filterRoughness(vec3 N, float roughness) {
    vec3 dndu = dFdx(N);
    vec3 dndv = dFdy(N);
    float variance = dot(dndu, dndu) + dot(dndv, dndv);
    float kernelRoughness = min(variance * 0.5, 0.18);  // Tuned threshold
...
// Height-correlated Smith GGX — more accurate than separable version
// Combines masking and shadowing with correlation factor
float GeometrySmithGGX(float NdotV, float NdotL, float roughness) {
    float a2 = roughness * roughness * roughness * roughness;  // a = r², a2 = a²
    float ggxV = NdotL * sqrt(NdotV * NdotV * (1.0 - a2) + a2);
    float ggxL = NdotV * sqrt(NdotL * NdotL * (1.0 - a2) + a2);
    return 0.5 / max(ggxV + ggxL, 0.0001);
}

// Spherical Gaussian approximation for Fresnel — faster and avoids pow()
...
// Multi-scatter energy compensation (Fdez-Agüera 2019)
// Adds the energy lost to inter-reflection at high roughness
vec3 multiscatterCompensation(vec3 F0, vec3 F, float NdotV, float roughness) {
    float Eavg = 1.0 - (roughness * 0.5);  // Approximate average directional albedo
    vec3 FssEss = F;  // Single-scatter contribution
...
vec2 vogelDiskSample(int sampleIndex, int samplesCount, float phi) {
    ...
    float theta = float(sampleIndex) * 2.39996323 + phi; // Golden angle = 2.39996323
...
float calcShadowCascade(sampler2DShadow shadowMap, vec4 fragPosLightSpace, vec3 N, vec3 lightDir) {
    ...
    // Contact Hardening approximation: scale radius by view depth
    float radiusScale = mix(2.0, 4.0, vViewDepth / 50.0);
    vec2 texelSize = 1.0 / textureSize(shadowMap, 0);
    
    // Vogel disk PCF with IGN dither
    float shadow = 0.0;
...
uniform float uFogDensity;  // 0.0 = no fog, 0.0005 = subtle, 0.002 = thick
...
float calcVolumetric(vec3 rayOrigin, vec3 rayDir, float rayLength, vec3 L) {
    ...
    // Dither start position to hide banding
    float noise = interleavedGradientNoise(gl_FragCoord.xy);
    vec3 currentPos = rayOrigin + stepVec * noise;
    
    float vLight = 0.0;
    for(int i = 0; i < NUM_STEPS; i++) {
        // Project to cascade 0
        ...
        if(projCoords.z <= 1.0 && projCoords.x > 0.0 && projCoords.x < 1.0 && projCoords.y > 0.0 && projCoords.y < 1.0) {
            ...
            vLight += shadow; // 1.0 = not in shadow
        }
        currentPos += stepVec;
    }
    ...
    return (vLight / float(NUM_STEPS)) * phase * rayLength * 0.02; // Density scale
}

/// CSM: select cascade by view depth, with smooth blending at transition
float calcShadowCSM(vec3 N, vec3 lightDir) {
    // Select cascade based on view depth
    float blendZone = uCascadeSplit * 0.1;  // 10% blend zone

    if (vViewDepth < uCascadeSplit - blendZone) {
        // Fully in cascade 0 (near, high detail)
        return calcShadowCascade(uShadowMap[0], vFragPosLightSpace[0], N, lightDir);
    }
    else if (vViewDepth < uCascadeSplit + blendZone) {
        // Blend zone between cascades
...
vec2 parallaxMapping(vec2 texCoords, vec3 viewDir) {
    ...
    // Interpolation for smooth result
...
// ── PBR Light Calculation (Production Quality) ──────────────
vec3 calcPBRLight(vec3 L, vec3 lightColor, vec3 N, vec3 V,
    ...
    // Specular (G already includes 1/(4·NdotV·NdotL) via height-correlated form)
...
void main() {
    // Parallax offset UV if height map present
    vec2 texCoord = vTexCoord;
    if (uUseHeightMap) {
        // Build TBN for parallax (same as getNormal)
        vec3 pos_dx = dFdx(vFragPos);
        vec3 pos_dy = dFdy(vFragPos);
        vec2 tex_dx = dFdx(vTexCoord);
        vec2 tex_dy = dFdy(vTexCoord);
        ...
        // Discard fragments outside [0,1] UV range
        if (texCoord.x > 1.0 || texCoord.y > 1.0 || texCoord.x < 0.0 || texCoord.y < 0.0)
            discard;
    }

    // Albedo (sRGB textures auto-linearized by hardware when using GL_SRGB8)
    vec3 albedo = uAlbedo;
    if (uUseAlbedoTex)
        albedo *= texture(uAlbedoTex, texCoord).rgb;

    // Metallic + Roughness (from texture or scalar)
    ...
    // Specular Anti-Aliasing: widen roughness on high-frequency normals
    ...
    Lo *= (1.0 - shadow * 0.85);  // Shadows never completely black (15% min light)
    ...
    if (uShadowEnabled && uVolumetricIntensity > 0.0) {
        ...
        // Limit max ray length for performance and to keep scattering localized
    ...
    // ── Ambient (IBL or hemisphere fallback) ──────────────────
    float NdotV = max(dot(N, V), 0.0);
    ...
    if (uIBLEnabled) {
        ...
        // Specular IBL: pre-filtered env + BRDF LUT
        vec3 R = reflect(-V, N);
    ...
    // ── Final color (PBR default) ──────────────────────────────
    vec3 color = ambient + Lo + emissive;

    // ── Render Mode Processing ──────────────────────────────
    if (uRenderMode == 1) {
        // TOON CEL-SHADING
...
inline const char* PHONG3D_VERT = PBR_VERT;
inline const char* PHONG3D_FRAG = PBR_FRAG;
